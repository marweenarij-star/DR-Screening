...
import json
import numpy as np
from train_efficientnet import EfficientNetB3Classifier, get_transforms, DRDataset, CONFIG as EFF_CONFIG

ROOT = Path(__file__).resolve().parents[1]
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

VAL_CSV = ROOT / 'APTOS' / 'cleaned' / 'val_clean.csv'
MODEL_PTH = ROOT / 'ai-service' / 'models' / 'dr_efficientnet_b3.pth'
OUT_JSON = ROOT / 'training' / 'calibration_report_effnet.json'
OUT_TEMP = ROOT / 'ai-service' / 'models' / 'effnet_temperature.json'


def load_model(path, device):
    model = EfficientNetB3Classifier(num_classes=EFF_CONFIG['num_classes'], pretrained=False)
    try:
...
if __name__ == '__main__':
    ...
    val_transform = get_transforms(is_training=False, image_size=EFF_CONFIG['image_size'])
    val_dataset = DRDataset(VAL_CSV, transform=val_transform)
    val_loader = DataLoader(val_dataset, batch_size=EFF_CONFIG['batch_size'], shuffle=False, num_workers=0)
